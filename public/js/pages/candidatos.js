...
import{$,esc,showToast,doubleConfirm}from"../common/ui.js";
...
if(session.role!=="candidate")await initResourcePage("candidatos");
  ...
  $("#candidate-profile").onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget));if(!await doubleConfirm({title:"Editar perfil",message:"Se guardarán los cambios de tu información profesional.",confirmLabel:"Guardar cambios"}))return;localStorage.setItem(profileKey,JSON.stringify(values));showToast("Perfil guardado.")};
}
